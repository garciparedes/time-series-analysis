...
semanal.residuals.raw$DATE
semanal.residuals.raw <- read.csv(paste0(BASE_DATA_PATH, 'prediccionesmultF.csv'))[1:520,]
semanal.residuals.raw$DATE <- mdy(semanal.residuals.raw$DATE)
semanal.residuals <- data.frame(index = semanal.residuals.raw$DATE,
                                values = semanal.residuals.raw$ERROR)
PlotTimeSeries(semanal.residuals, seasonality = 52, lags = 104) %>%
  { save_plot(paste0(BASE_IMG_PATH, 'semanal-residuals.png'), .,
                     base_aspect_ratio = 1.75, base_height = 8) }
