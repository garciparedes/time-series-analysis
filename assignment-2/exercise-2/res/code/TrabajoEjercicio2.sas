...
proc expand data=semanal out=semanal4 from=week.6 to=week4.6 ; /* cada 4 los suma y pone la fecha correspondinte.*/
id date;
...
proc esm data=semanal4 outfor=predicSEASONAL;
  id date interval = week4.6;
  forecast tubersem / model = SEASONAL;
run;

proc esm data=semanal4 outfor=predicadd;
  id date interval = week4.6;
  forecast tubersem / model = ADDWINTERS;
run;


proc esm data=semanal4 outfor=predicmult;
  id date interval = week4.6;
...
proc esm data = semanal4 outfor = predic lead = 6;
	id date interval=week4.6;
	forecast tubersem / model = WINTERS;
run;
...
DATA semanal4reducido;
  set semanal4;
  if _n_ < 119;
run;

proc esm data = semanal4reducido outfor = predicseasonal lead = 12  seasonality = 13;
  id date interval = week4.6;
  forecast tubersem / model = SEASONAL;
run;

proc esm data = semanal4reducido outfor = predicadd lead = 12  seasonality = 13;
  id date interval = week4.6;
  forecast tubersem / model = ADDWINTERS;
run;


proc esm data = semanal4reducido outfor = predicmult lead = 12 seasonality = 13;
  id date interval = week4.6;
  forecast tubersem / model = WINTERS;
run;
...
data errores;
  merge semanal4 predicseasonal predicadd predicmult;
...
PROC FORECAST DATA = semanal4reducido OUT = RESULfor interval = week4.6
